...
class GetGHalls:
    def __init__(self, filename=None, table_key=None):
        ...
        self.worksheet = self.get_worksheet('Площадки')
        self.results = []
        self.results_msg = []
    ...
    def get_worksheet(self, name=None):
        """Проверка """
        if name is not None:
            if isinstance(name, int):
                return self.table.get_worksheet(name)
            if isinstance(name, str):
                return self.table.worksheet(name)
        raise 'Рабочий лист не выбран'
